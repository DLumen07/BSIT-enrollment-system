...
try {
    ...
    $lookupStmt = $conn->prepare('SELECT code FROM subjects WHERE id = ? LIMIT 1');
    if (!$lookupStmt) {
        throw new Exception('Failed to prepare subject lookup: ' . $conn->error);
    }
    $lookupStmt->bind_param('i', $subjectId);
    $lookupStmt->execute();
    $subjectCode = '';
    $lookupStmt->bind_result($subjectCode);
    $found = $lookupStmt->fetch();
    $lookupStmt->close();
    ...
    if (!$deleteStmt) {
        throw new Exception('Failed to prepare subject delete: ' . $conn->error);
    }
