...
if (!empty($data->user_id) && !empty($data->name) && !empty($data->department)) {
    ...
    if ($instructor->update()) {
        ...
        echo json_encode(array("message" => "Instructor profile was updated."));
    } else {
        http_response_code(503);
        echo json_encode(array("message" => "Unable to update instructor profile."));
    }
} else {
    http_response_code(400);
    echo json_encode(array("message" => "Unable to update instructor profile. Data is incomplete."));
}
?>